#!/usr/bin/env python3
"""
verify_v9_kp.py — Phase 9 Physics Verification
...
import torch
import numpy as np
from train_v9_dual_mono import IonisDualMono, MonotonicMLP, DNN_DIM, SFI_IDX, KP_PENALTY_IDX

# --- CONFIG ---
MODEL_PATH = "models/ionis_v9_dual_mono.pth"
DEVICE = torch.device("mps")


def get_test_vector(sfi_normalized, kp_penalty):
    """Build a 15-feature test vector.

    Features 0-12: DNN inputs (geometry, time, interactions)
    Feature 13: sfi (Sun Sidecar)
    Feature 14: kp_penalty (Storm Sidecar)
    """
    ...
    features = [
        ...
        -0.8,    # hour_cos
        # NO raw ssn here — removed to prevent leakage
        0.0,     # az_sin
        1.0,     # az_cos
        0.1,     # lat_diff
        ...
        0.5,     # season_sin
        0.5,     # season_cos
        0.2,     # ssn_lat_interact (shape modulation only)
        1.0,     # day_night_est (Daytime)
        0.5,     # band_sfi_interact (shape modulation only)
        # Sidecar inputs
        sfi_normalized,  # Sun Sidecar (index 13)
        kp_penalty,      # Storm Sidecar (index 14)
    ]
