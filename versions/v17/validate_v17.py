#!/usr/bin/env python3
"""
validate_v17.py — V17 RBN Grid-Enriched vs VOACAP comparison on Step I paths

Queries 1M contest paths from validation.step_i_paths, runs V17 inference,
denormalizes to dB, applies mode thresholds, and reports recall.

V17: WSPR + RBN + Contest + DXpedition (51M rows, real machine-measured SNR)
...
def engineer_features(tx_lat, tx_lon, rx_lat, rx_lon, freq_mhz, month, hour_utc, ssn):
    """Build 13 features matching V17 training EXACTLY."""
    n = len(tx_lat)
    ...
    # Stack features EXACTLY matching training
...
def main():
    print("=" * 70)
    print("  IONIS V17 RBN Grid-Enriched — Step I Validation")
    print("=" * 70)
    print()
    ...
    # Get normalization constants from checkpoint
    snr_mean = checkpoint.get('snr_mean', 0.0)
    snr_std = checkpoint.get('snr_std', 1.0)
    print(f"  SNR normalization: mean={snr_mean:.3f}, std={snr_std:.3f}")
    print()
    ...
    # Denormalize using checkpoint's snr_mean and snr_std
    print("Denormalizing predictions to dB...")
    predictions_db = predictions_sigma * snr_std + snr_mean
    ...
    print(f"  V17 vs VOACAP:   {delta_voacap:+.2f} pp")
    print(f"  V17 vs V15:      {delta_v15:+.2f} pp")
    print(f"  V17 vs V16:      {delta_v16:+.2f} pp")
    print()
    ...
    # Return exit code based on minimum recall threshold
    if recall >= 80.0:
        print("  STATUS: PASS (recall >= 80%)")
        return 0
    else:
