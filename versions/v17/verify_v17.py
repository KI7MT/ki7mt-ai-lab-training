#!/usr/bin/env python3
"""
verify_v17.py — IONIS V17 Physics Verification
...
V17 uses per-source per-band Z-score normalization. Model outputs are in σ units.
"""
...
def test_physics():
    ...
    print()
    print(f"  NOTE: V17 outputs are in Z-score (σ) units.")
    ...
    print(f"  SNR at Kp 0 (Quiet):  {d_quiet['predicted']:+.3f} σ")
    print(f"  SNR at Kp 9 (Storm):  {d_storm['predicted']:+.3f} σ")
    ...
    print(f"\n  Storm Cost (Kp 0→9): {storm_cost:+.3f} σ")
    ...
    print(f"  SNR at SFI 70:   {d_low['predicted']:+.3f} σ")
    print(f"  SNR at SFI 200:  {d_high['predicted']:+.3f} σ")
    ...
    print(f"\n  Sun Benefit (SFI 70→200): {sun_benefit:+.3f} σ")
    ...
    if all_pass:
        ...
        print()
        print("  V17 RBN Grid-Enriched Model Physics:")
        print(f"    Kp 0→9 storm cost:   {storm_cost:+.3f} σ")
        print(f"    SFI 70→200 benefit:  {sun_benefit:+.3f} σ")
        ...
        print()
        print("  V17 trained on WSPR + RBN + Contest + DXpedition (51M rows)")
    else:
