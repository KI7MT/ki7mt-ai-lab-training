...
Phase 6 changes from Phase 5.2:
    ADDED:   kp_penalty = 1.0 - kp/9.0 (pre-computed in ClickHouse)
    ADDED:   Monotonic constraint: first-layer weights for kp_penalty clamped >= 0
    KEPT:    Raw sfi, kp features (independent solar channels)
    KEPT:    IFW sampling weights (applied via per-sample weighted MSE loss)

Architecture: 18 -> 512 -> 256 -> 128 -> 1 (Mish, no BatchNorm)
"""
...
INPUT_DIM = 18
MODEL_DIR = 'models'
MODEL_FILE = 'ionis_v6_monotonic.pth'
PHASE = 'Phase 6: Monotonic Kp Constraint (18 features, IFW weighted MSE)'

DEVICE = torch.device("mps" if torch.backends.mps.is_available() else "cpu")

# kp_penalty is index 17 (last feature, 0-indexed).
...
FEATURES = [
    ...
    'day_night_est', 'sfi', 'kp', 'band_sfi_interact',
...
log = logging.getLogger('ionis-v6')
...
def engineer_features(df):
    """Compute 18 normalized features from raw CSV columns. Returns (N, 18) float32."""
    n = len(df)
    ...
    kp = df['kp'].values.astype(np.float32)
    ...
    return np.column_stack([
        ...
        f_sfi, f_kp, f_band_sfi, f_sfi_dist, f_kp_penalty,
    ]).astype(np.float32)
...
class IonisMish(nn.Module):
    """Mish MLP: 18 -> 512 -> 256 -> 128 -> 1 (no BatchNorm)."""
...
def main():
    ...
    log.info("Engineering 18 features...")
    t0 = time.perf_counter()
    ...
    log.info(f"Model: 18 -> 512 -> 256 -> 128 -> 1  ({params:,} parameters)")
    ...
    # Training
    log.info("Training started (Monotonic Kp Constraint Active)")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for bx, by, bw in train_loader:
            ...
            # Clamp first-layer weights for kp_penalty (idx 17) to >= 0.
            # Prevents the model from learning "storms are good for HF".
            with torch.no_grad():
