#!/usr/bin/env python3
"""
oracle_v12.py — IONIS V16 Propagation Oracle

Production inference interface for IONIS V16 model.
...
class IonisOracle:
    """IONIS V16 Propagation Oracle — production inference interface."""

    def __init__(self, model_path: str = MODEL_PATH):
        self.model = IonisV16Gate().to(DEVICE)
        ...
        self.metadata = {
            ...
            'architecture': checkpoint.get('architecture', 'IonisV16Gate'),
...
ROBUSTNESS_TESTS = [
    ...
    # TST-506: Checkpoint integrity (updated 2026-02-11 V16 Contest)
    {"name": "Checkpoint RMSE", "test": "checkpoint_rmse",
     "expected": 0.8602, "tolerance": 0.01},

    {"name": "Checkpoint Pearson", "test": "checkpoint_pearson",
     "expected": 0.4873, "tolerance": 0.01},
...
# NOTE: V16 outputs Z-scores (σ), not dB. Values labeled as "dB" are actually σ.
REGRESSION_TESTS = [
    # TST-801: Reference prediction (V16 outputs ~-0.3σ for this path)
    {"name": "Reference: W3→G 20m", "test": "reference",
     "params": {"lat_tx": 39.14, "lon_tx": -77.01, "lat_rx": 51.50, "lon_rx": -0.12,
                "freq_mhz": 14.0, "sfi": 150, "kp": 2, "hour": 14},
     "expected_snr": -0.3, "tolerance": 0.5},
]
...
def run_test_suite():
    ...
    print("=" * 70)
    print("  IONIS V16 Oracle Test Suite")
    print("=" * 70)
...
def interactive_mode():
    ...
    print("=" * 70)
    print("  IONIS V16 Propagation Oracle — Interactive Mode")
    print("=" * 70)
