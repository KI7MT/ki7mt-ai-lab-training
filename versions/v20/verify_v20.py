...
def test_physics():
    ...
    print(f"  RMSE: {checkpoint.get('val_rmse', 0):.4f} sigma ({checkpoint.get('val_rmse', 0) * DB_PER_SIGMA:.2f} dB equiv)")
    print(f"  Pearson: {checkpoint.get('val_pearson', 0):+.4f}")
    print()
    print(f"  NOTE: V20 outputs are in Z-score (sigma) units.")
    print(f"        Approximate dB conversion: x{DB_PER_SIGMA:.1f}")
    ...
    storm_effect_quiet = model.get_storm_effect(1.0, DEVICE)
    storm_effect_storm = model.get_storm_effect(0.0, DEVICE)

    print(f"\nScenario: SFI 150 (fixed), 20m Band, Mid-Day")
    print(f"  SNR at Kp 0 (Quiet):  {d_quiet['predicted']:+.3f} sigma ({d_quiet['predicted'] * DB_PER_SIGMA:+.1f} dB)")
    print(f"  SNR at Kp 9 (Storm):  {d_storm['predicted']:+.3f} sigma ({d_storm['predicted'] * DB_PER_SIGMA:+.1f} dB)")
    print(f"  Storm Sidecar at Kp 0: {storm_effect_quiet:+.3f} sigma")
    print(f"  Storm Sidecar at Kp 9: {storm_effect_storm:+.3f} sigma")
    ...
    print(f"\n  Storm Cost (Kp 0->9): {storm_cost:+.3f} sigma ({storm_cost * DB_PER_SIGMA:+.1f} dB)")
    ...
    sun_effect_low = model.get_sun_effect(70.0 / 300.0, DEVICE)
    sun_effect_high = model.get_sun_effect(200.0 / 300.0, DEVICE)

    print(f"\nScenario: Kp 2 (fixed), 20m Band, Mid-Day")
    print(f"  SNR at SFI 70:   {d_low['predicted']:+.3f} sigma ({d_low['predicted'] * DB_PER_SIGMA:+.1f} dB)")
    print(f"  SNR at SFI 200:  {d_high['predicted']:+.3f} sigma ({d_high['predicted'] * DB_PER_SIGMA:+.1f} dB)")
    print(f"  Sun Sidecar at SFI 70:  {sun_effect_low:+.3f} sigma")
    print(f"  Sun Sidecar at SFI 200: {sun_effect_high:+.3f} sigma")
    ...
    print(f"\n  Sun Benefit (SFI 70->200): {sun_benefit:+.3f} sigma ({sun_benefit * DB_PER_SIGMA:+.1f} dB)")
    ...
    x_random[:, SFI_IDX] = torch.rand(1000, device=DEVICE)
    x_random[:, KP_PENALTY_IDX] = torch.rand(1000, device=DEVICE)
    ...
    # ── SUMMARY ──
    print("\n" + "=" * 60)
    print("V20 PHYSICS VERIFICATION SUMMARY")
    print("=" * 60)
    ...
    # V20 validation thresholds from config
    ...
    if all_pass:
        ...
        if sun_benefit > 0:
            print(f"    Storm/Sun ratio:     {storm_cost / sun_benefit:.1f}:1")
        ...
        print()
        kp_ok = storm_cost >= kp_min
        sfi_ok = sun_benefit >= sfi_min
        print(f"  V20 vs Thresholds:")
        print(f"    Kp storm cost >= {kp_min} sigma:  {'PASS' if kp_ok else 'BELOW TARGET'} ({storm_cost:+.3f})")
        print(f"    SFI benefit >= {sfi_min} sigma:   {'PASS' if sfi_ok else 'BELOW TARGET'} ({sun_benefit:+.3f})")
    else:
