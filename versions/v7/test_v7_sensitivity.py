...
# --- 1. Model Architecture (must match Phase 7 training) ---
...
checkpoint = torch.load("models/ionis_v7_lobotomy.pth", weights_only=False, map_location=device)
input_dim = checkpoint['input_dim']
model = IonisMish(input_dim=input_dim).to(device)
model.load_state_dict(checkpoint['model_state'])
model.eval()

print(f"Model loaded: {input_dim} features, 512 -> 256 -> 128 hidden")
...
# --- 5. Helper (17-feature model, Phase 7: raw kp removed) ---
...
#   sfi, band_sfi_interact, sfi_dist_interact, kp_penalty
...
def make_input(distance_km, freq_hz, hour, month, azimuth,
    ...
    Raw kp is NOT a model input (removed in Phase 7 lobotomy).
    Kp enters only through kp_penalty = 1.0 - kp/9.0, which is
    guarded by the monotonic constraint.
    """
    ...
    # Kp enters ONLY through kp_penalty (monotonic-constrained)
...
print(f"\n{'='*62}")
print(f"  IONIS V2 - SNR Sensitivity Analysis (Phase 7: Kp Lobotomy)")
print(f"  Reference path: {TX_GRID} -> {RX_GRID} ({REF_DISTANCE:.0f} km, 20m WSPR)")
print(f"  Solar features: SSN, SFI (raw), kp_penalty (monotonic-constrained)")
print(f"  Raw kp REMOVED — sole Kp channel is kp_penalty = 1.0 - kp/9.0")
print(f"  Interactions: band_sfi, sfi_dist")
