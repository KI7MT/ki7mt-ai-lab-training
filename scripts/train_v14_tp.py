#!/usr/bin/env python3
"""
train_v14_tp.py — IONIS V14-TP (Terrestrial Physics)

V14-TP uses balloon-filtered WSPR signatures for clean terrestrial physics.
Balloon/HAP callsigns (>50 grids/day velocity) are excluded from training.

Data sources:
    - wspr.signatures_v2_terrestrial: Balloon-filtered WSPR signatures
    - rbn.dxpedition_signatures: 91K signatures (already terrestrial — skimmers are ground-based)
...
Checkpoint: models/ionis_v14_tp.pth
"""
...
MODEL_FILE = "ionis_v14_tp.pth"
PHASE = "V14-TP: Terrestrial Physics (balloon-filtered)"
VERSION = "v14-tp"
...
RBN_SAMPLE = None           # All RBN DXpedition signatures (no sampling)

# Upsampling: repeat RBN data to balance contribution
RBN_UPSAMPLE_FACTOR = 50    # Same as V13
...
log = logging.getLogger('ionis-v14-tp')
...
def load_combined_data():
    """Load and combine WSPR (terrestrial) + RBN DXpedition signatures."""
    log.info(f"Connecting to ClickHouse at {CH_HOST}:{CH_PORT}...")
    client = clickhouse_connect.get_client(host=CH_HOST, port=CH_PORT)

    # ── WSPR Terrestrial Signatures (balloon-filtered) ──
    ...
    # ── RBN DXpedition Signatures (already terrestrial) ──
...
def main():
    log.info(f"IONIS V14-TP | {PHASE}")
    log.info(f"Device: {DEVICE} | Epochs: {EPOCHS} | Batch: {BATCH_SIZE}")
    log.info(f"Data sources: wspr.signatures_v2_terrestrial + rbn.dxpedition_signatures")
    log.info(f"Normalization: per-source per-band Z-score")
    log.info(f"RBN upsampling: {RBN_UPSAMPLE_FACTOR}x")
    log.info(f"Balloon filtering: ENABLED (V2 terrestrial signatures)")
    log.info("")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save({
                ...
                'data_sources': ['wspr.signatures_v2_terrestrial', 'rbn.dxpedition_signatures'],
                'normalization': 'per-source per-band Z-score',
                'balloon_filtering': True,
