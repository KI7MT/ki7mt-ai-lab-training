#!/usr/bin/env python3
"""
train_v10_final.py — IONIS V2: Phase 10 Final (Geography Reintegration)

Sequential Residual Training - Phase B:
  1. Phase A (Naked Physics): Sidecars learned SFI/Kp physics with DNN frozen
  2. Phase B (This file): DNN learns geography while sidecars maintain physics

Architecture: IonisDualMono
    DNN:          11 -> 512 -> 256 -> 128 -> 1 (geography & time ONLY)
...
Key constraints:
    - DNN has ZERO solar information (starvation protocol)
    - Sidecar weights clamped 0.5-2.0 (prevents collapse AND explosion)
    - Sidecar fc2.bias is learnable (relief valve for calibration)
    - Sidecar fc1.bias is frozen (maintains activation shape)
    - Differential LR: DNN 1e-5 (slow), Sidecars 1e-3 (fast)

Expected outcomes:
    - RMSE: 2.52 -> ~2.35 dB (as DNN explains geography)
    - Pearson: ~0.0001 -> ~0.30 (as geography noise drops)
    - SFI+: stays ~+0.5 dB (physics preserved)
    - Kp9-: stays ~+1.1 dB (physics preserved)
"""
...
VAL_SPLIT = 0.2
MODEL_DIR = 'models'
MODEL_FILE = 'ionis_v10_final.pth'
PHASE = 'Phase 10 Final: Geography Reintegration (DNN slow, sidecars clamped)'

# Turbo Loader settings for MPS performance
NUM_WORKERS = 12
PIN_MEMORY = True
PREFETCH_FACTOR = 4
...
# Feature indices (Starvation Protocol from Phase 9):
...
#
# DNN has ZERO solar/storm information - all physics flows through sidecars.
...
log = logging.getLogger('ionis-v10')
...
def main():
    ...
    log.info("Architecture: IonisDualMono (Phase 10: Geography Reintegration)")
    ...
    log.info("  Differential LR: DNN 1e-5 (slow) | Sidecars 1e-3 (fast)")
    ...
    log.info(f"  Initial: Sun bias={sun_fc2_b:.1f}, Storm bias={storm_fc2_b:.1f}")
    ...
    log.info("Initializing Optimizer: Phase 10 - Geography Reintegration")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        # Brief sidecar check every 10 epochs
