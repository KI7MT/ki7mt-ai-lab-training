#!/usr/bin/env python3
"""
train_v14_tp_v2.py — IONIS V14-TP-v2 (Terrestrial Physics, Corrected Filter)

V14-TP-v2 uses CORRECTED balloon-filtered WSPR signatures (V2).
This is a clean A/B comparison against V14-TP which used the flawed V1 filter.

Data source:
    - wspr.signatures_v2_terrestrial: 93.3M signatures (surgical 20K balloon removal)
    - NO DXpedition synthesis (that's V15)

Filter comparison:
    - V1 (V14-TP): 89.9M signatures (3.4M false positives removed)
    - V2 (V14-TP-v2): 93.3M signatures (only 20K actual balloons removed)

Architecture: IonisV12Gate (unchanged — 203,573 params)
Target: Z-normalized median_snr
Weight: spot_count

Checkpoint: models/ionis_v14_tp_v2.pth
"""
...
MODEL_FILE = "ionis_v14_tp_v2.pth"
PHASE = "V14-TP-v2: Terrestrial Physics (balloon-filtered)"
VERSION = "v14-tp-v2"
...
log = logging.getLogger('ionis-v14-tp-v2-v2')
...
def load_wspr_data():
    """Load WSPR terrestrial signatures only (no DXpedition)."""
    ...
    # ── Normalize SNR per-band (WSPR only) ──
    log.info("Applying per-band Z-score normalization...")
    ...
    log.info(f"Dataset: {len(wspr_X):,} rows (WSPR only)")

    # Free DataFrame
    del wspr_df
    gc.collect()

    return wspr_X, wspr_snr.reshape(-1, 1), wspr_w.reshape(-1, 1), date_range


# ── Training ─────────────────────────────────────────────────────────────────

def main():
    log.info(f"IONIS V14-TP-v2 | {PHASE}")
    log.info(f"Device: {DEVICE} | Epochs: {EPOCHS} | Batch: {BATCH_SIZE}")
    log.info(f"Data source: wspr.signatures_v2_terrestrial (WSPR only)")
    log.info(f"Normalization: per-band Z-score")
    log.info(f"Balloon filtering: V2 CORRECTED (surgical 20K removal)")
    log.info("")

    # ── Load Data ──
    X_np, y_np, w_np, date_range = load_wspr_data()
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save({
                ...
                'data_sources': ['wspr.signatures_v2_terrestrial'],
                'normalization': 'per-band Z-score',
    ...
    log.info("")
    log.info("=== V14-TP vs V13 ===")
    log.info(f"V13 (includes balloon):  Pearson +0.2865")
    log.info(f"V14-TP (terrestrial only): Pearson {best_pearson:+.4f}")
