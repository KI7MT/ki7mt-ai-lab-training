...
# --- 1. Model Architecture (must match Phase 8 training) ---
class IonisHybrid(nn.Module):
    ...
    def forward(self, x):
        x_deep = x[:, :-1]
        x_phys = x[:, -1:]
        return self.dnn(x_deep) + self.sidecar(x_phys)


# --- 2. Load Model ---
device = torch.device("mps" if torch.backends.mps.is_available() else "cpu")

checkpoint = torch.load("models/ionis_v8_sidecar.pth", weights_only=False, map_location=device)
...
# --- 5. Helper (17-feature model, Phase 8: PIML sidecar) ---
...
# Features 0-15 -> DNN, Feature 16 (kp_penalty) -> sidecar
...
def make_input(distance_km, freq_hz, hour, month, azimuth,
    ...
    Features 0-15 feed the DNN. Feature 16 (kp_penalty) feeds the
    sidecar — a single positive-constrained linear weight that bypasses
    all hidden layers.
    """
    ...
    # Kp enters ONLY through sidecar (positive-constrained linear bypass)
...
print(f"\n{'='*62}")
print(f"  IONIS V2 - SNR Sensitivity Analysis (Phase 8: PIML Sidecar)")
print(f"  Reference path: {TX_GRID} -> {RX_GRID} ({REF_DISTANCE:.0f} km, 20m WSPR)")
print(f"  Solar features: SSN, SFI (raw) -> DNN")
print(f"  kp_penalty -> Sidecar (linear bypass, weight >= 0)")
