#!/usr/bin/env python3
"""
train_v15_diamond.py — IONIS V15 "Diamond Burn" (Clean Base + DXpedition)

V15 synthesizes the Gold Layers:
    - Base Layer: wspr.signatures_v2_terrestrial (93.3M clean WSPR signatures)
    - DX Layer: rbn.dxpedition_signatures (91K rare-path signatures, 50x upsampled)

This is the convergence of:
    - V14-TP-v2's clean terrestrial physics (balloon-filtered, +0.590σ RMSE)
    - V13's geographic recall (DXpedition data teaching rare paths)

Objective: Achieve Pearson +0.30 range with Recall 85%+ range.

Architecture: IonisV12Gate (unchanged — 203,573 params)
Target: Z-normalized median_snr
Weight: spot_count

Checkpoint: models/ionis_v15_diamond.pth
"""
...
MODEL_FILE = "ionis_v15_diamond.pth"
PHASE = "V15 Diamond: Clean Base + DXpedition"
VERSION = "v15-diamond"
...
log = logging.getLogger('ionis-v15-diamond')
...
def main():
    log.info(f"IONIS V15 Diamond | {PHASE}")
    log.info(f"Device: {DEVICE} | Epochs: {EPOCHS} | Batch: {BATCH_SIZE}")
    log.info(f"Data sources: WSPR terrestrial (clean) + RBN DXpedition ({RBN_UPSAMPLE_FACTOR}x)")
    ...
    log.info("")
    log.info("=== V15 Diamond Comparison ===")
    log.info(f"V14-TP-v2 (WSPR only):   Pearson +0.2959, Recall 76.0%")
    log.info(f"V13 (WSPR + DX, balloon): Pearson +0.2865, Recall 85.3%")
    log.info(f"V15 (Clean + DX):        Pearson {best_pearson:+.4f}")
