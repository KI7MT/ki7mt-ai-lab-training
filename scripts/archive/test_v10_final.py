...
# --- 1. Model Architecture (must match Phase 10 training) ---
class MonotonicMLP(nn.Module):
    """Small MLP with monotonically increasing output."""
    def __init__(self, hidden_dim=8):
        super().__init__()
        self.fc1 = nn.Linear(1, hidden_dim)
        self.fc2 = nn.Linear(hidden_dim, 1, bias=True)  # bias=True for Phase 10
...
checkpoint = torch.load("models/ionis_v10_final.pth", weights_only=False, map_location=device)
...
# --- 5. Helper (13-feature model, Phase 10 - Nuclear Option) ---
def make_input(distance_km, freq_hz, hour, month, azimuth,
               tx_lat, tx_lon, rx_lat, rx_lon, ssn, sfi, kp):
    """Build 13-feature vector: 11 DNN + 1 sfi + 1 kp_penalty.

    V10 NUCLEAR OPTION: DNN gets ZERO solar info.
    No interaction terms - all solar/storm signal through sidecars.
    """
    ...
    # 13 features: 11 DNN (geography & time) + sfi + kp_penalty
    return torch.tensor(
...
print(f"\n{'='*62}")
print(f"  IONIS V2 - SNR Sensitivity Analysis (Phase 10: Geography Reintegration)")
