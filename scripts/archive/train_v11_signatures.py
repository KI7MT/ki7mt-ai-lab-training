#!/usr/bin/env python3
"""
train_v11_signatures.py — IONIS V11 "Platinum Burn" on Aggregated Signatures

Step F retrain: same IonisV11Gate architecture (203,573 params), but trained
...
Architecture: unchanged from V11 final (IonisV11Gate)
Features: same 13 features, derived from signature table columns

Checkpoint: models/ionis_v11_signatures.pth (V11 final checkpoint preserved)
"""
...
MODEL_FILE = "ionis_v11_signatures.pth"
PHASE = "V11 Signatures: Platinum Burn (aggregated median training)"
...
log = logging.getLogger('ionis-v11-sig')
...
def main():
    ...
    log.info("Architecture: IonisV11Gate (trunk+3heads+2sidecars) — UNCHANGED")
    ...
    log.info("")
    log.info("=== STEP F COMPARISON ===")
    log.info(f"V11 Final (raw spots):     Pearson +0.2376, RMSE 2.48 dB")
    log.info(f"V11 Signatures (medians):  Pearson {best_pearson:+.4f}, RMSE {best_rmse:.2f} dB")
    improvement = best_pearson - 0.2376
    log.info(f"Pearson change: {improvement:+.4f} "
             f"({'IMPROVED' if improvement > 0 else 'REGRESSED'})")
